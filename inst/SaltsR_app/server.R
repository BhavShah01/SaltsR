...
server <- function(input, output) {
  ...
  output$sel_sample <- renderUI({
    textInput("sel_sample", "Sample name", value = "Unique name to link to metadata")
  })
  output$sel_dry_g <- renderUI({
    numericInput("sel_dry_g", "Dry sample mass (g)", value = 0.801, min = 0)
  })
  output$sel_water_ml <- renderUI({
    numericInput("sel_water_ml", "Water (ml)", value = 100, min = 0)
  })
  output$sel_chloride <- renderUI({
    numericInput("sel_chloride", "Chloride (ppm)", value = 14.651, min = 0)
  })
  output$sel_nitrate <- renderUI({
    numericInput("sel_nitrate", "Nitrate (ppm)", value = 17.339, min = 0)
  })
  output$sel_sulfate <- renderUI({
    numericInput("sel_sulfate", "Sulfate  (ppm)", value = 39.923, min = 0)
  })
  output$sel_sodium <- renderUI({
    numericInput("sel_sodium", "Sodium (ppm)", value = 2.027, min = 0)
  })
  output$sel_potassium <- renderUI({
    numericInput("sel_potassium", "Potassium (ppm)", value = 2.04, min = 0)
  })
  output$sel_calcium <- renderUI({
    numericInput("sel_calcium", "calcium (ppm)", value = 49.809, min = 0)
  })
  output$sel_magnesium <- renderUI({
    numericInput("sel_magnesium", "Magnesium (ppm)", value = 0.581, min = 0)
  })
  ...
  output$salts_x_ECOS <- renderDT({
    salts_results() |>
      ...
      DT::datatable(extensions = c("Buttons"),
                    list(dom = 'Bt', buttons = list("copy", "excel")))
  })
